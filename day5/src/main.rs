...
impl Kid {
    pub fn parse_row(csv_row: &str) -> Result<Kid, &'static str> {
        ...
        // 🎁 Your code here! 🎁
        let mut splitted = csv_row.split(',');
        let name = splitted.next().ok_or("missing field")?.to_string();
        let good_deeds: u32 = splitted.next().ok_or("missing field")?.parse().map_err(|_| "invalid number")?;
        let bad_deeds: u32 = splitted.next().ok_or("missing field")?.parse().map_err(|_| "invalid number")?;
